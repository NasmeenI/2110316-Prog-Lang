...
object four {
  ...
  private def moreThanHalf(l1: List[Any], l2: List[Any]): Boolean = {
    if(l1.isEmpty) false
    else if(l1.length % 2 == 0) find(l1, l2) >= l1.length / 2
    else find(l1, l2) > l1.length / 2
  }

  private def find(l1: List[Any], l2: List[Any]): Int = {
    if(l1.isEmpty) 0
    else if(findOne(l2, l1.head)) 1 + find(l1.tail, l2)
    else find(l1.tail, l2)
  }

  private def findOne(l: List[Any], x: Any): Boolean = {
    if(l.isEmpty) false
    else if(l.head == x) true
    else findOne(l.tail, x)
  }
}
